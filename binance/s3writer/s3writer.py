...
def s3_bucket_folders(data, _symbol, year, month, day):
    return r'true-alpha/exchange=Binance/'+data+r'/symbol='+_symbol+'/year='+str(year)+'/month=' + str(month) + '/day=' + str(day) + '/'

def s3_bucket_raw_data_folders(year, month, day):
    return r'raw-data/exchange=Binance'+r'/year='+str(year)+'/month=' + str(month) + '/day=' + str(day) + '/'
...
def process_raw_line(line):
    ...
    if has_minute_increased(old_raw_data_timestamp, utc_timestamp) == "Flush":
        folders = s3_bucket_raw_data_folders(year, month, day)
        print(f'Flashing to s3://{bucket_name}/{folders}{seq}')
