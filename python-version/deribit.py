...
if sys.argv[1] == 'BTC-PERPETUAL':
    symbol_ws = sys.argv[1]
elif sys.argv[1] == 'ETH-PERPETUAL':
    symbol_ws = sys.argv[1]
...
def s3_bucket_folders(data, symbol_ws, year, month, day):
    return r'true-alpha' + '/exchange=Deribit/' + data + '/symbol=' + symbol_ws + '/year=' + str(year) + '/month=' + str(month) + '/day=' + str(day) + '/'
...
def on_message(ws, message):
    ...
    seq = data['params']['data']['change_id']
...
def on_open(ws):
    ...
    msg = \
        {"jsonrpc": "2.0",
         ...
         "id": 42,
         "params": {
             "channels": ["book." + symbol_ws + ".10.10.100ms"]}
         }
