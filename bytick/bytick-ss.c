...
{
	bytick_t *bin = lws_container_of(sul, bytick_t, sul_hz);
	bytick_t *g = bin;
...
bytick_transfer_callback(void *userobj, lws_ss_tx_ordinal_t ord, uint8_t *buf, size_t *len,
	     int *flags)
{
	bytick_t *bin = (bytick_t *)userobj;
	lws_ss_state_return_t r = LWSSSSRET_OK;
	bytick_t *g = bin;

	lwsl_debug("bytick_transfer_callback");
...
static lws_ss_state_return_t
bytick_receive_callback(void *userobj, const uint8_t *in, size_t len, int flags)
{
	bytick_t *bin = (bytick_t *)userobj;
...
static lws_ss_state_return_t
bytick_state(void *userobj, void *h_src, lws_ss_constate_t state,
...
LWS_SS_INFO("bytick", bytick_t)
	.rx           = bytick_receive_callback,
	.tx           = bytick_transfer_callback,
	.state        = bytick_state,
};
