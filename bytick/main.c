...
extern const lws_ss_info_t ssi_bytick_t;
...
int main(int argc, const char **argv)
{
	...
	{
		lwsl_debug("LWS bytick client topic not specified");
		return	-1;
	}
	...
	// {
	// 	lwsl_debug("LWS bytick client invalid topic %s\n", topic);
	// 	return	-1;
	// }

	lwsl_debug("LWS bytick client for topic: %s\n", topic);
	...
	if (lws_ss_create(cx, 0, &ssi_bytick_t, NULL, NULL, NULL, NULL)) {
